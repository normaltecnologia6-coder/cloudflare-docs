...
Send an HTTP request directly to the container process.
...
- <span id="static-outbound"></span>**`static outbound`** — catch-all handler
  for outbound requests not matched by any host-specific rule.

- <span id="static-outboundbyhost"></span>**`static outboundByHost`** — per-host
  handlers. Keys are exact hostnames such as `"api.example.com"` or an IP
  address.

- <span id="static-outboundhandlers"></span>**`static outboundHandlers`** —
  named handlers that can be switched on at runtime via
  [`setOutboundHandler()`](#setoutboundhandler) and
  [`setOutboundByHost()`](#setoutboundbyhost). Use these when you want to change
  which handler is active based on runtime conditions.

For more information about outbound matching behavior and fallbacks, refer to
[Handle outbound traffic](/containers/platform-details/outbound-traffic/).

Each handler receives the outgoing request, the Worker `env`, and an `OutboundHandlerContext`.

**Handler context**:

- `containerId` - ID of the container instance making the outbound request.
- `className` - Name of your `Container` subclass.
- `params` - Optional runtime params passed through [`setOutboundHandler()`](#setoutboundhandler) or [`setOutboundByHost()`](#setoutboundbyhost).
...
Set the active catch-all outbound handler at runtime.
...
- `methodName` - Name of a handler defined in [`static outboundHandlers`](#static-outboundhandlers).
- `params` - Optional runtime params exposed to the handler as `ctx.params`.

**Returns**: `Promise<void>`.

Throws if `methodName` is not defined in [`static outboundHandlers`](#static-outboundhandlers).

#### `setOutboundByHost`

Route a specific hostname to a named handler at runtime.
...
- `hostname` - Hostname or `ip:port` pair to intercept.
- `methodName` - Name of a handler defined in [`static outboundHandlers`](#static-outboundhandlers).
- `params` - Optional runtime params exposed to the handler as `ctx.params`.
...
- `handlers` - Map of hostname to handler name, or to an object with `method` and optional `params`.

**Returns**: `Promise<void>`.

Throws if any referenced handler name is not defined in [`static outboundHandlers`](#static-outboundhandlers).

#### `removeOutboundByHost`

Remove a runtime hostname override.
...
- `hostname` - Hostname to remove from the runtime override list.

**Returns**: `Promise<void>`.

Removes a runtime hostname override, falling back to any matching [`static outboundByHost`](#static-outboundbyhost) handler.
...
export class MyContainer extends Container {
	static outboundHandlers = {
		signedFetch: async (request) => {
			return fetch(request);
		},
		kvAccess: async (request, env) => {
			const key = new URL(request.url).pathname.slice(1);
			const value = await env.DEMO_KV.get(key);
			return new Response(value ?? "", { status: value ? 200 : 404 });
		},
	};

    async configureOutboundAccess() {
    	await this.setOutboundHandler("signedFetch");
    	await this.setOutboundByHost("my.kv", "kvAccess");

    	await this.setOutboundByHosts({
    		"api.example.com": "signedFetch",
    		"my.kv": "kvAccess",
    	});

    	await this.removeOutboundByHost("my.kv");
    }
